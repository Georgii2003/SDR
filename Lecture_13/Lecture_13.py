...
xf1=np.fft.fft(xrec**4,2048) 
K=2*np.pi*np.linspace(-0.5,0.5,2048) 
plt.figure(3)
plt.plot(K, np.abs(np.fft.fftshift(xf1)))
arg=np.argmax(np.fft.fftshift(xf1)) 
offf=K[arg]/4  

xrec_coarse=xrec*np.exp(-1j*offf)  
plt.figure(4)
...
for i in range(iter-1):
    ...
    phi_error[i]=np.imag(dds_mult[i])
